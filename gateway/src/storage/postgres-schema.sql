-- Postgres schema for gateway durable state.
-- All mapping tables are Firm-scoped (scentic_firm_id column).
-- No document contents, raw signer emails, or secrets stored.
--
-- Multi-instance safety:
-- - Nonces use INSERT ... ON CONFLICT DO NOTHING for atomic replay prevention
-- - Idempotency keys use INSERT ... ON CONFLICT for atomic duplicate prevention
-- - Outbox uses SELECT ... FOR UPDATE SKIP LOCKED for safe concurrent processing
-- - All mappings have UNIQUE constraints preventing duplicates
...
-- Nonces: atomic replay prevention via ON CONFLICT DO NOTHING
...
-- Idempotency keys: atomic duplicate prevention via ON CONFLICT
...
-- Outbox: safe concurrent processing via FOR UPDATE SKIP LOCKED
